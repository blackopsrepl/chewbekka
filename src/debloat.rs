...
pub async fn dissect_subtlety(content: &str) -> String {
    let prompt = format!("Analyze this for hidden intent, no softening: {}", content);
...
pub async fn strip_jargon(content: &str) -> String {
    let prompt = format!("Remove all vague, corporate, or inclusive terms. State facts only: {}", content);
    let debloated_text = chat_completion("chewbekka", &prompt, "user").await;
...
mod tests {
    ...
    async fn test_strip_jargon() {
        let content = "Lorem ipsum dolor sit amet. We love inclusive, agile and demure synergy.";
