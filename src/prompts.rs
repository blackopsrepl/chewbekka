...
const GENERATE_README: &str =
    "Generate a comprehensive, industry standard README.md file for the following code: ";
...
pub fn get_prompt(task: &str, content: &str) -> String {
    match task {
        ...
        "docugen" => format!("{}{}", GENERATE_README, content),
...
async fn test_get_prompt() {
    ...
    assert_eq!(
        prompt,
        format!("Generate a task list from this document: {}", content)
    );
